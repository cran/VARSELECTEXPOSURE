...
#' @return List containing (1) the estimated Average Treatment Effect, (2) estimated Relative Treatment Effect, (3) summary of the selected model, and (4) the first 6 rows of the data frame containing backward-selected covariates.
#' @references
...
BACKWARD_EXPOSURE = function(Data){
  ...
  for(j in 1:p){
    ...
    if(pval > 0.05){
      BACKWARD.DATA = BACKWARD.DATA[,-(new.index+2)]
      D0 = D1
