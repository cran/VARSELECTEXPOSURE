...
#' @return List containing (1) the estimated Average Treatment Effect, (2) estimated Relative Treatment Effect, (3) summary of the selected model, and (4) the first 6 rows of the data frame containing forward-selected covariates.
#' @references
...
FORWARD_EXPOSURE = function(Data){
  ...
  for(j in 1:p){
    ...
    if(pval < 0.05){
      ...
      colnames(FORWARD.DATA)[VAR.NUM] = names(Data)[VAR.NUM]
      VAR.NUM = VAR.NUM + 1
      D0 = D1
